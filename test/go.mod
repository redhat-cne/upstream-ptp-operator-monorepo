module github.com/k8snetworkplumbingwg/ptp-operator/test

go 1.24.0

// Use main branch's api/ and pkg/ for test compatibility with latest controller-runtime.
// base-repo contains the main branch checkout on the VM.
replace github.com/k8snetworkplumbingwg/ptp-operator => ../../base-repo

// openshift/client-go and openshift/api replace directives must mirror the base-repo's
// go.mod because Go does not inherit replace directives from replaced modules.
// Without these, openshift/client-go@v0.0.1 resolves to a version built against
// structured-merge-diff/v4, while k8s.io/apimachinery@v0.34.x uses v6, causing
...
require (
	github.com/k8snetworkplumbingwg/ptp-operator v0.0.0-00010101000000-000000000000
